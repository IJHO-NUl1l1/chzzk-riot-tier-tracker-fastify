...
export async function tftSpectatorRoute(app: FastifyInstance) {
  app.get('/api/riot/tft/spectator/:region/:puuid', async (request, reply) => {
    ...
    try {
      const url = `https://${regionHost}/tft/spectator/v1/active-games/by-summoner/${encodeURIComponent(puuid)}`;
      const response = await axios.get(url, { headers: { 'X-Riot-Token': apiKey } });
      return reply.send(response.data);
    } catch (err: any) {
      if (err.response?.status === 404) {
        return reply.send({ inGame: false, message: 'Summoner is not in an active game' });
      }
